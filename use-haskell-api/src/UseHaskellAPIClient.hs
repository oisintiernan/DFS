...
getREADME :: Maybe FilePath -> ClientM FileData
...
files :: Maybe FilePath -> ClientM FileHere
init :: ClientM Init
-- | The following provides the implementations of these types
-- Note that the order of the functions must match the endpoints in the type API from UseHaskell.hs

(loadEnvVars :<|> getREADME :<|> storeMessage :<|> searchMessage :<|> performRestCall :<|> files :<|> init) = client restAPI
